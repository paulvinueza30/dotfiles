...
webapp_profile="--profile-directory=Clean"
...
webapps["special:NeetCode"]="https://neetcode.io/"
webapps["special:N8N"]="https://n8n.paulvinueza.dev"
...
patterns["special:N8N"]="n8n.paulvinueza.dev"
...
handle_workspace_lifecycle() {
  ...
  if [[ -n ${webapps[$new_with_prefix]} ]]; then
    ...
    if ! hyprctl clients -j | jq -e --arg p "$pattern" '.[] | select(.class | contains($p))' >/dev/null 2>/dev/null; then
      hyprctl dispatch exec "[workspace $new_with_prefix silent] omarchy-launch-or-focus-webapp $pattern ${webapps[$new_with_prefix]} $webapp_profile" 2>/dev/null &
    fi
